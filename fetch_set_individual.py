...
Outputs each set to its own .txt file.
...
def fetch_all_set_codes():
    """
    Fetch all available set codes from Scryfall API.
    Returns a list of all set codes sorted alphabetically.
    """
    logger.info("Fetching all available sets from Scryfall...")
    ...
    try:
        ...
        # Filter out token sets and special sets (only include regular sets)
        set_codes = [
            s.get("code").lower() 
            for s in sets 
            if s.get("type") not in ("token", "promo") and s.get("code")
        ]
        
        set_codes.sort()
        logger.info(f"Found {len(set_codes)} sets: {', '.join(set_codes[:10])}... (showing first 10)")
        
        return set_codes
    except requests.exceptions.RequestException as e:
        logger.error(f"Error fetching sets from Scryfall: {e}")
        return []
...
def load_config():
    ...
    Supports special value "all" or "all_sets" to fetch all sets from Scryfall.
    Returns tuple: (set_codes_list, download_images)
    """
    ...
    if not config_file.exists():
        ...
        logger.warning("Using default values: set_codes=[], download_images=False")
        return set_codes, download_images
    ...
    try:
        with open(config_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if key == "set_codes":
                    # Check for special "all" or "all_sets" value
                    if value.lower() in ("all", "all_sets"):
                        set_codes = fetch_all_set_codes()
                    else:
...
def parse_arguments():
    ...
    Supports:
    - No arguments: use config.txt
    - --config: explicitly use config.txt
    - --build-all: fetch and build all sets from Scryfall
    - --build=SETCODE: build only the specified set code
    - --download-images: enable image downloading (can be combined with any mode)
    """
    parser = argparse.ArgumentParser(
        description="Fetch Magic: The Gathering cards from Scryfall and format for Lackey CCG"
    )
    ...
    mode_group.add_argument(
        "--build-all",
        action="store_true",
        help="Fetch and build all sets from Scryfall"
    )
    mode_group.add_argument(
        "--build",
        metavar="SETCODE",
        type=str,
        help="Build only the specified set code (e.g., --build=eoe)"
    )
    mode_group.add_argument(
        "--config",
        action="store_true",
        help="Use config.txt (default if no options specified)"
    )
...
def determine_sets_and_images(args):
    """
    Determine which sets to fetch and whether to download images
    based on command-line arguments.
    Returns tuple: (set_codes, download_images)
    """
    download_images = args.download_images
    
    if args.build_all:
        logger.info("Mode: Build all sets from Scryfall")
        set_codes = fetch_all_set_codes()
        return set_codes, download_images
    
    elif args.build:
        logger.info(f"Mode: Build single set ({args.build})")
        set_codes = [args.build.lower()]
        return set_codes, download_images
    
    else:
        # Default: use config.txt (--config is implicit if no other mode specified)
        logger.info("Mode: Using config.txt")
        config_set_codes, config_download_images = load_config()
        
        # Command-line flag overrides config file
        if args.download_images:
            download_images = True
        else:
            download_images = config_download_images
        
        return config_set_codes, download_images


# Global variables loaded from arguments or config
args = parse_arguments()
SET_CODES, DOWNLOAD_IMAGES = determine_sets_and_images(args)
...
def write_set_file(set_code, cards):
    """
    Write formatted cards to a file named after the set code.
    Each set gets its own .txt file in the sets directory.
...
def main():
    ...
    Each set is output to its own .txt file.
    """
    logger.info("=" * 60)
    ...
    if not SET_CODES:
        logger.error("No set codes defined!")
        logger.error("Usage:")
        logger.error("  python fetch_set_individual.py                    # Uses config.txt")
        logger.error("  python fetch_set_individual.py --config           # Uses config.txt (explicit)")
        logger.error("  python fetch_set_individual.py --build-all        # Fetches all sets from Scryfall")
        logger.error("  python fetch_set_individual.py --build=eoe        # Builds only the 'eoe' set")
        logger.error("  python fetch_set_individual.py --build=eoe --download-images  # Build with images")
        sys.exit(1)
    
    logger.info(f"Fetching {len(SET_CODES)} set(s): {', '.join(SET_CODES)}")
    logger.info(f"Output mode: Individual files (one per set)")
